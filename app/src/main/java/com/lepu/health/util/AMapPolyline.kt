...
/**
 *
 * 负责在AMaps上进行渐变的类[PolylineOptions]。 *计算颜色并绘制渐变的主要功能是[drawPolyline]
 */
class AMapPolyline(private val map: AMap, private val job: CompletableJob) :
    CoroutineScope {
    ...
    fun setPolylineOptions(polylineOptions: PolylineOptions): AMapPolyline {
        this.polylineOptions = polylineOptions
        return this
    }

    fun setDelayTime(delayTime: Long): AMapPolyline {
        this.delayTime = delayTime
        return this
    }

    fun setStartColor(startColor: Int): AMapPolyline {
        this.startColor = startColor
        return this
    }

    fun setEndColor(endColor: Int): AMapPolyline {
    ...
    /**
     * 根据起始和结束颜色生成渐变线，从而负责在[AMap]多段线上绘制渐变。
     */
    fun drawPolyline(latLngRouteList: List<LatLng>, onDrawFinished: () -> (Unit)) {
        ...
        launch {
             ...
             */
//            val simplifiedList: List<LatLng> = douglasCompress(latLngRouteList, 1.0)
            val simplifiedListSize = latLngRouteList.size.toFloat()
             ...
             */
            for (index in 0 until latLngRouteList.size - 1) {
                builder.include(latLngRouteList[index])
                /**
                ...
                gradientPoly.add(
                    copyPolylineOptions(polylineOptions)
                        .color(gradientColor)
                        .add(latLngRouteList[index])
                        .add(latLngRouteList[index + 1])
                )
            }
            /**
    ...
    private fun copyPolylineOptions(originPolylineOptions: PolylineOptions): PolylineOptions {
        with(originPolylineOptions) {
            ...
            copyPolylineOptions.isDottedLine = isDottedLine
            copyPolylineOptions.dottedLineType = dottedLineType
            copyPolylineOptions.lineCapType(lineCapType)
            copyPolylineOptions.lineJoinType(lineJoinType)
...
inline fun AMap.zoomCameraWithAnimation(
    cameraUpdate: CameraUpdate,
    crossinline onFinishAnimation: () -> Unit
) {
    setOnMapLoadedListener {
        animateCamera(cameraUpdate, 1000, object : AMap.CancelableCallback {
